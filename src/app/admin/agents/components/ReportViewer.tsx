...
function RenderValue({ value, depth = 0 }: { value: unknown; depth?: number }) {
    ...
    return (
      <div className={depth > 0 ? "space-y-2" : "space-y-3"}>
        {entries.map(([key, val]) => {
          ...
          if (typeof val === "string" && !val.includes("\n") && val.length < 120) {
            return (
              <div key={key} className="flex gap-2 items-baseline flex-wrap">
                <span className="text-[11px] font-medium text-white/25 shrink-0 min-w-[80px]">
                  {label}:
                </span>
          ...
          return (
            <div key={key}>
              <p className="text-[11px] font-medium text-white/30 uppercase tracking-wider mb-1.5">
                {label}
              </p>
  ...
  return <span className="text-xs text-white/30">{String(value)}</span>;
}
              ...
              {agentReports.map((report) => (
                <div key={report.id}>
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] text-white/30 line-clamp-2">
                      {report.summary}
                    </p>
                  </button>
